...
OrdersList::~OrdersList()
{
    ...
    delete orders; // Delete the memory for the vector itself.
}
...
OrdersList::OrdersList(const OrdersList& oldList)
{
    ...
    orders = new vector<Order*>(listSize); // Dynamically create vector of same size.
...
void OrdersList::addOrder(Order* o)
{
    ...
    {
        cout <<"This is a null pointer! Adding operation failed! \n" << endl;
    }
...
vector<Order*>* OrdersList::getListOfOrders()
{
    return orders;
}
...
{                                                            // to RHS
    ...
    this->orders = new vector<Order*>(listSize); // Reallocate memory for vector of size RHS
...
{
    if (orderType == "deploy" || orderType == "Deploy")
        return new deploy;
    else if (orderType == "advance" || orderType == "Advance")
        return new advance;
    else if (orderType == "bomb" || orderType == "Bomb")
        return new bomb;
    else if (orderType == "blockade" || orderType == "Blockade")
        return new blockade;
    else if (orderType == "airlift" || orderType == "Airlift")
        return new airlift;
    else if (orderType == "negotiate" || orderType == "Negotiate")
        return new negotiate;
    else
        return nullptr;
}

int OrdersList::listSize()
{
    return orders->size();
}
